...
import type { Host, HostMetrics, HostHistoryPoint } from "@/lib/types";
...
export function HostsBrowser({
  initialHosts, initialMetrics, initialHistory, authed,
}: {
  initialHosts: Host[];
...
function HostCard({
  host, metrics, history, authed, onEdit, onDelete,
}: {
  host: Host;
  metrics?: HostMetrics;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <Link href={`/hosts/${host.id}`} className="font-medium truncate hover:underline">{host.name}</Link>
